...
(function(){
	...
	let MenuTpl =
		'<div id="menu_{{_namespace}}_{{_name}}" class="menu{{#align}} align-{{align}}{{/align}} ">' +
			...
			'<div class="head" ><ion-icon name="ellipsis-horizontal" class="ic"></ion-icon><span style=" background-image: linear-gradient(to right,rgb(112, 32, 136), #be3cca,rgb(195, 74, 199), #963cca,#963cca); padding-left: 20px; padding-right: 20px;  padding-top: 2px;   padding-bottom: 2px; border-radius: 0 0 12px 12px;   box-shadow: rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -2px 0px inset;">{{{title}}}</span></div>' +
		
				'<div class="menu-items">' + 
					'{{#elements}}' +
						'<div class="menu-item {{#selected}}selected{{/selected}}">' +
							'{{{label}}}{{#isSlider}} <span class="right-label {{#selected}}right-label-select{{/selected}}"><i class="material-icons middle {{#selected}}right-label-select{{/selected}}">keyboard_arrow_left</i>{{{sliderLabel}}}<i class="material-icons middle {{#selected}}right-label-select{{/selected}}">keyboard_arrow_right</i></span>{{/isSlider}}' +
						'</div>' +
					'{{/elements}}' +
				'</div>'+
	
				'<div class="head-fed"></div>' +
			'</div>' +
		'</div>';
